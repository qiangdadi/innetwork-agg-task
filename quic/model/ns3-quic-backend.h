...
class MyClientEndpoint:public Ns3QuicEndpointBase,
public Ns3QuicChannelBase::Notifier{
public:
    MyClientEndpoint(Ns3QuicBackendBase* backend,bool bidi_channel);
    ~MyClientEndpoint() override;
    ...
    //from Ns3QuicChannelBase::Notifier
    void OnChannelDestroy(Ns3QuicChannelBase *channel) override;
    ns3::Ptr<ns3::CircularBuffer> GetBuffer ();
private:
    Ns3QuicChannelBase *channel_=nullptr;
    bool bidi_channel_=false;
};

class MyClientBackend:public Ns3QuicBackendBase{
public:
    MyClientEndpoint* endpoint_;
    MyClientBackend(Ns3QuicAlarmEngine* engine,bool bidi_channel=true);
    ~MyClientBackend() override{}
    Ns3QuicEndpointBase* CreateEndpoint(Ns3QuicSessionBase *session) override;
    //void Send (const char* buffer,size_t len, bool fin);
private:
    bool bidi_channel_ = true;
};

class MyServerEndpoint:public Ns3QuicEndpointBase,
public Ns3QuicChannelBase::Notifier{
public:
    MyServerEndpoint(Ns3QuicBackendBase* backend,bool bidi_channel);
    ~MyServerEndpoint() override;
    size_t Send (const char* buffer,size_t len, bool fin);
    //from Ns3QuicEndpointBase
    void OnIncomingBidirectionalStream(Ns3TransportStream *stream) override;
    void OnIncomingUnidirectionalStream(Ns3TransportStream *stream) override;
...
class MyServerBackend:public Ns3QuicBackendBase{
public:
    MyServerEndpoint* endpoint_;
    MyServerBackend(Ns3QuicAlarmEngine* engine,bool bidi_channel=true);
    ~MyServerBackend() override{}
    Ns3QuicEndpointBase* CreateEndpoint(Ns3QuicSessionBase *session) override;
    //void Send (const char* buffer,size_t len, bool fin);
private:
    bool bidi_channel_ = true;
