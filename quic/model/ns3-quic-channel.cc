...
MyChannel::MyChannel(Ns3TransportStream *stream):
stream_(stream){
    cbuffer = new ns3::CircularBuffer();
}

size_t
MyChannel::Send (const char* buffer,size_t len, bool fin) {
    size_t wlen = stream_->Write(buffer,len,fin);
    NS_LOG_INFO ("[Mychannel:Send] send success, length = " << len);
    return wlen;
}
...
MyChannel::OnCanRead(){
    ...
    bool fin=false;
    size_t rlen = stream_->Read(buffer, 1024 * 8, fin);
    cbuffer->write (reinterpret_cast<uint8_t*>(buffer), rlen);
    delete [] buffer;
    NS_LOG_INFO("[MyChannel:read] read success, length = " << rlen);
}

void 
MyChannel::OnCanWrite(){}

void 
MyChannel::OnDestroy(){
    NS_LOG_INFO("[MyChannel:OnDestroy]");
    if(notifier_){
        notifier_->OnChannelDestroy(this);
    }
}
